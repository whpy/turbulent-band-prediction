    ...
    def reset_parameters(self):
        ...
        for key, value in weight_dict.items():
            if key == 'weight_ih_l0':
                ...
                #abs_eigs = (torch.eig(w_hh)[0] ** 2).sum(1).sqrt() # this usage has been deprecated
                e,_ = torch.linalg.eig(w_hh)
                abs_eigs = torch.norm( torch.unsqueeze(e,dim=0), dim=0 ) 
...
#    print(hy_.device)
#    here we introduce the random factor
    hy = (1 - leaking_rate) * hidden + leaking_rate * hy_  +0.001*(torch.rand(hy_.shape)-0.5).to(hy_.device)
    return hy
